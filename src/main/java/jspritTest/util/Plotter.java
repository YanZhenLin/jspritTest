...
public class Plotter {
    ...
    //instantiates the activities reference
    private void retrieveActivities(VehicleRoutingProblem vrp) throws NoLocationFoundException {
        ...
        //add the vehicles
        for (Vehicle v : vrp.getVehicles()) { //get all the vehicles from the vrp instance and iterate throught the vehicles
            Coordinate start_coordinate = getCoordinate(v.getStartLocation().getCoordinate()); //get the start coordinate
            if (start_coordinate == null) throw new NoLocationFoundException(); //throw exception if the start coordinate is null
            XYDataItem item = new XYDataItem(start_coordinate.getX() * scalingFactor, start_coordinate.getY() * scalingFactor);
            markItem(item, Activity.START); //mark the Item as a Start 
            activities.add(item);

            //if the current vehicle's start location does not equal its end location's, mark the end coordinate as an xyDataItem
        ...
        //services, pickups, deliveries are all instanceof job interface
